...
auto unique_notif_name(const std::string &notification_id, const std::string &topic_name) {
  return notification_id + "_" + topic_name;
}

// from list of bucket topics, find the one that was auto-generated by a notification
auto find_unique_notification(const rgw_pubsub_bucket_topics &bucket_topics, const std::string &notification_id,
                              const std::string &topic_name) {
  auto it = bucket_topics.topics.find(unique_notif_name(notification_id, topic_name));
  return it != bucket_topics.topics.end() ?
         std::optional<std::reference_wrapper<const rgw_pubsub_topic_filter>>(it->second):
         std::nullopt;
}
...
int RGWPubSub::Bucket::get_notification_by_id(const DoutPrefixProvider *dpp, const std::string &notification_id,
  ...
  auto iter = find_unique_notification(bucket_topics, notification_id, topic_name);
  if (!iter) {
...
{
  return remove_notification_inner(dpp, topic_name, y);
}

int RGWPubSub::Bucket::remove_notification_inner(const DoutPrefixProvider *dpp, const std::string &topic_name, optional_yield y) const
{
  RGWObjVersionTracker objv_tracker;
  rgw_pubsub_bucket_topics bucket_topics;
